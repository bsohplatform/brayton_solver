    ...
    def Solver(self, first, second, settings, outputs, max_p, process_case):
        ...
        while a: 
            ...
            for i in range(2):
                ...
                first['hot_out'].T = second['hot_in'].T + settings.hot_T_pp
                first['hot_out'].h = CP.PropsSI('H','T',first['hot_out'].T,'P',first['hot_out'].p,first['hot_out'].fluid)
                first['cold_out'].T = second['cold_in'].T - settings.cold_T_pp
                ...
                if process_case <= 2:
                    first['cold_in'].m = second['cold_in'].q / (first['cold_out'].h - first['cold_in'].h)
                    first['cold_out'].m = first['cold_in'].m
                else:
                    first['hot_in'].m = second['hot_in'].q / (first['hot_in'].h - first['hot_out'].h)
        ...
        if process_case == 0:
            second['hot_in'].h = second['hot_out'].h - outputs.hot_Q/second['hot_out'].m
            second['hot_in'].T = CP.PropsSI('T','H',second['hot_in'].h,'P',second['hot_in'].p,second['hot_in'].fluid)
        elif process_case == 1:
            second['hot_out'].h = second['hot_in'].h + outputs.hot_Q/second['hot_in'].m
            second['hot_out'].T = CP.PropsSI('T','H',second['hot_out'].h,'P',second['hot_out'].p,second['hot_out'].fluid)
        elif process_case == 2:
            second['hot_in'].m = outputs.hot_Q/(second['hot_out'].h - second['hot_in'].h)
        elif process_case == 3:
            ...
            second['cold_in'].m = outputs.cold_Q/(second['cold_in'].h - second['cold_out'].h)
...
if __name__ == "__main__":
    ...
    hot_in_T = 500+273.15
    hot_in_p = 101300.0
    hot_out_T = 510+273.15
    hot_out_p = 101300.0
    hot_m = 1.0
    
    cold_in_T = 300+273.15
    cold_out_T = 290+273.15
    cold_in_p = 101300.0
    cold_out_p = 101300.0
    cold_m = 0.0
    ...
    settings = Settings(hot_dp = 0.001, hot_T_pp=5, cold_dp = 0.0, cold_T_pp = 5.0, ihx_cold_dp = 0.0, ihx_hot_dp = 0.0, ihx_eff = 0.9, comp_eff = 0.75, expand_eff = 0.85, gen_eff=0.99)
    outputs = Outputs()
